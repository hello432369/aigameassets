...
- In every animated body grid prompt, require the subject body to stay centered in each cell, full body inside the central 60% to 70% safe area, consistent scale across cells, stable feet/bottom anchor line when applicable, no shadows anywhere (no drop shadow, no ground shadow, no character shadow), and no limbs, weapons, hair, capes, dust, muzzle flashes, or detached FX crossing cell edges.
...
- no shadows anywhere — no drop shadow, no ground shadow, no character shadow (shadows on magenta background cause chroma-key artifacts and unclean cutout)
